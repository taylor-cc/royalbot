     ...
     getalltable: function (SSID) {
        return new Promise(function (resolve, reject) {
            fs.readFile('credentials.json', async (err, content) => {
                ...
                try {
                    var oauth = await getauth(JSON.parse(content));
                    var data = await toget(oauth, SSID, 'List' + '!A1:F31', 'COLUMNS');
                    // resolve(data);
                    var sheetname = getSheetName()
     ...
     getotable: function (SSID,listname) {
        return new Promise(function (resolve, reject) {
            fs.readFile('credentials.json', async (err, content) => {
                ...
                try {
                    var oauth = await getauth(JSON.parse(content));
                    var data = await toget(oauth, SSID, listname + '!A1:F31', 'COLUMNS');
                    // resolve(data);
                    var sheetname = getSheetName()
                    var data2 = await toget(oauth, SSID, sheetname + '!A1:T33', 'ROWS');
                    resolve([data, data2]);
 ...
 getotablebyRow: function (SSID,list) {
        return new Promise(function (resolve, reject) {
            fs.readFile('credentials.json', async (err, content) => {
                ...
                try {
                    var oauth = await getauth(JSON.parse(content));
                    var data = await toget(oauth, SSID, list + '!A1:E31', 'ROWS');
                    resolve(data);
                }
                catch (err) {
                    reject(err);
