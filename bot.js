...
var objlist = { "0": "集刀","集": "集刀","1": "一王", "2": "二王", "3": "三王", "4": "四王", "5": "五王", "一": "一王", "二": "二王", "三": "三王", "四": "四王", "五": "五王" }
...
client.on('message', async message => {
    ...
    if (message.content.substring(0, 1) === "!" || message.content.substring(0, 1) === "！") {
		...
		return;
	...
	else if (command === '報一'|| command === 'add1' ) {
                queue.push(async () => {
 			var str = ''; //組合回報訊息(args)
                         var list= '一王';
			if (args.length >= 1) {
                        ...
                        message.reply('請回覆指定王&組合/傷害').then(d_msg => { d_msg.delete(5000) });
                        return;
                    }
        ...
        else if (command === '報二' || command === 'add2') {
                queue.push(async () => {
 			var str = ''; //組合回報訊息(args)
                         var list= '二王';
			if (args.length >= 1) {
	...
	else if (command === '報三'|| command === 'add3' ) {
                queue.push(async () => {
 			var str = ''; //組合回報訊息(args)
                         var list= '三王';
			if (args.length >= 1) {
	...
	else if (command === '報四' || command === 'add4') {
                queue.push(async () => {
 			var str = ''; //組合回報訊息(args)
                         var list= '四王';
			if (args.length >= 1) {
	...
	else if (command === '報五' || command === 'add5') {
                queue.push(async () => {
 			var str = ''; //組合回報訊息(args)
                         var list= '五王';
			if (args.length >= 1) {
			...
			content = [[str]];
			 ...
			 if (args.length >= 2) {
                ...
                queue.push(async () => {
                    ...
                    try {
                        if (args.length != 1) {
                            message.channel.send('請輸入要收回的王 ex: !delte 5');
                            return;
                        }
                ...
                queue.push(async () => {
                    ...
                    try {
                        ...
                        if (args.length != 2) {
                            message.channel.send('請輸入要收回的王 ex: !delte @成員 5');
                            return;
                        }
                                                ...
                                                
			{
                ...
                queue.push(async () => {
                    try {
                        if (args.length != 1) {
                            message.channel.send('請輸入要進場的王 ex: !進 1 || 集刀請打 !進 0 ');
                            return;
                        }
                        list  = objlist[args[0].substring(0,1)]
                        var tables = await gapi.getotable(chlist[message.channel.id],list );
